...
func TestNewPAExternalAPIClient(t *testing.T) {
	...
	monitor := NewPAExternalAPIClient("1234key", "https://serverpa")
...
`
	...
	defer ts.Close()
	fmt.Print(ts.URL)
	monitor := NewPAExternalAPIClient("1234key", ts.URL)
...
func TestNewPAExternalAPIClient_GetMonitorInfos_NoResponse(t *testing.T) {
	monitor := NewPAExternalAPIClient("1234key", "https://nourl.com")
	_, err := monitor.GetMonitorInfos("ALL")
	assert.NotNil(t, err)
}

func TestNewPAExternalAPIClient_GetMonitorInfos_BadUrl(t *testing.T) {
	monitor := NewPAExternalAPIClient("1234key", "::?s::s&t::::oto\x20--notanurl.com")
	_, err := monitor.GetMonitorInfos("ALL")
	assert.NotNil(t, err)
}
